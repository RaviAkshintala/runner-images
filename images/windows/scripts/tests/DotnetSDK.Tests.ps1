    ...
    foreach ($version in $dotnetVersions) {
        Context "Dotnet $version" {
            ...
            It "SDK $version is available" -TestCases $dotnet {
                (dotnet --list-sdks | Where-Object { $_ -match "^\Q${dotnetVersion}\E\.[0-9]+\.[0-9]+" }).Count | Should -BeGreaterThan 0
            }

            It "Runtime $version is available" -TestCases $dotnet {
                (dotnet --list-runtimes | Where-Object { $_ -match "^\Q${dotnetVersion}\E\.[0-9]+\.[0-9]+" }).Count | Should -BeGreaterThan 0
            }
        }
    }
