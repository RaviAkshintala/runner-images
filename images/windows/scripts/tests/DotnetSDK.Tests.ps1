    ...
    foreach ($version in $dotnetVersions) {
        Context "Dotnet $version" {
            ...
            It "SDK $version is available" -TestCases $dotnet {
                (dotnet --list-sdks | Where-Object { $_.StartsWith("$version.") }).Count | Should -BeGreaterThan 0
            }

            It "Runtime $version is available" -TestCases $dotnet {
                (dotnet --list-runtimes | Where-Object { $_.StartsWith("$version.") }).Count | Should -BeGreaterThan 0
            }
        }
    }
